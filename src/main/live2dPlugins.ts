...
interface Ilive2d extends Electron.BrowserWindow {
  showMessage?: IShowMessageFunc;
}

const initlive2dPlugins = (mainWindow: BrowserWindow) => {
  global.__plugins = {};
  ...
  const installPlugin = async (name: string, code: string) => {
    ...
    const func = new Function('live2d', 'app', 'fetch', 'module', code);

    const live2d: Ilive2d = mainWindow;

    live2d.showMessage = showMessage;
...
export default initlive2dPlugins;
